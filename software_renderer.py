...
class Software_Renderer(object):
        ...
        for x in range(min_bounding_box.x, max_bounding_box.x + 1):
            for y in range(min_bounding_box.y, max_bounding_box.y + 1):
                ...
                if self.active_tex:  
                    ...
                    colour2 = self.active_shader_no_tex(
                        self,                        
                        barycentric_coords=(b1, b2, b3),     
                        pos_coords=(x, y),   
                        varying_normals=(normal_A, normal_B, normal_C),
                    ...
                    try:
                        if z > self.zBuffer[y][x]:
                            # print('about to draw point at: (x,y)' + str(x) + ', ' + str(y) + ', ' + '. Normalized: ' + str(self.glGetNormalizedXCoord(x)) + str(self.glGetNormalizedYCoord(y)))
                            self.glVertex(self.glGetNormalizedXCoord(x), self.glGetNormalizedYCoord(y), colour2)
                            self.zBuffer[y][x] = z
                    except:
                        pass

    def glDrawStars(self):
        for x in range(self.viewport_width):
            for y in range(self.viewport_height):
                if (998 < randint(0, 1000)):
                    self.glVertex(self.glGetNormalizedXCoord(x), self.glGetNormalizedYCoord(y), color(255,255,255))
    ...
    def glSetGouradShaderNoTexture(self, obj, **kwargs):
        ...
        #     round(255 * dot_product(normal, VERTEX_3(0,0,kwargs['intensity']))) for normal in (normal_A, normal_B, normal_C)
        # ]
        ...
        vect = VERTEX_3(norm_x, norm_y, norm_z)  
        texture_color = color(255, 255, 255) 

        if (self.renderIs == 'planet'):
            # colors
            base_color = color(93, 43, 22)
            lighter_color = color(100, 51, 21)
            darker_color = color(74, 33, 19)
            ...
            if (bool(random.getrandbits(1))):
                if (bool(random.getrandbits(1))):
                    texture_color = lighter_color
                else:
                    texture_color = darker_color

            if ((randint(500, 680) < x < randint(600, 700)) and (randint(500, 600) < y < randint(600, 700))):
                texture_color = purple_color
            ...
            if ((randint(850, 900) < x < randint(950, 1000)) and (randint(780, 820) < y < randint(820, 900))):
                texture_color = top_color

        if (self.renderIs == 'moon'):
            # colors
            base_color = color(88, 88, 88)
            lighter_color = color(92, 92, 92)
            darker_color = color(36, 36, 36)            
        ...
        tex_intensity = dot_product(vect, VERTEX_3(0,0,1))
